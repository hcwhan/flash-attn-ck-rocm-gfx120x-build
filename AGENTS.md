...
**Windows / gfx1201 / PyTorch 2.12.0+rocm7.14.0 / Python 3.12** 推理专用 FlashAttention 2 CK wheel。版本 pin 见 **`VERSION.lock.json`**。仅 **CI**（`windows-2022` 干净 runner），无本地编译入口。编排脚本为 **TypeScript**（Node 26 + `tsx`）。
...
| 入口 | 职责 |
|------|------|
| `scripts/cli.ts` | 统一 CLI（`npx tsx scripts/cli.ts <cmd>`） |
...
| Action | 用途 |
|--------|------|
| `01.fa-rocm-toolchain` | 安装 Python / MSVC / PyTorch / ROCm |
