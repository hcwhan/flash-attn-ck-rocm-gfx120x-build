...
| `skip_cache_restore` | `false` | 设为 `true` 时跳过 cache restore（仅 lookup 探测 cache-hit；compile 成功后仍 save） |
...
- Ninja cache **仅在 `06.compile` 成功时 save**；编译失败不写入（job 超时/取消同样不写）。cache 命中且 compile 成功时，save 前先删除旧条目再刷新。
