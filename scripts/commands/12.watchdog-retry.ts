import { spawnSync } from "node:child_process";
import { requireGithubActionsEnv } from "../lib/require-env.js";
import { evaluateParallelWatchdogRetry } from "../lib/watchdog-abort-meta.js";

const WORKFLOW_FILES = {
  serial: "build-fa2-ck-gfx120x-serial.yml",
  parallel: "build-fa2-ck-gfx120x-parallel.yml",
} as const;

const MAX_RETRY_COUNT = 8;
const DISPATCH_ATTEMPTS = 3;
const DISPATCH_RETRY_INTERVAL_MS = 60_000;
const CANCEL_WAIT_MS = 300_000;

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function dispatchRetryWorkflow(
  buildVariant: keyof typeof WORKFLOW_FILES,
  nextRetryCount: number,
): boolean {
  const repo = requireGithubActionsEnv("GITHUB_REPOSITORY");
  const ref = requireGithubActionsEnv("GITHUB_REF_NAME");
  const maxJobs = requireGithubActionsEnv("MAX_JOBS").trim();
  const useCache = requireGithubActionsEnv("USE_CACHE") === "true";
  const publishRelease = requireGithubActionsEnv("PUBLISH_RELEASE") === "true";
  const ckDisableBwd = requireGithubActionsEnv("CK_FMHA_DISABLE_BWD") === "1";

  // gh -F coerces numeric-looking values to numbers; string workflow inputs need -f.
  const result = spawnSync(
    "gh",
    [
      "api",
      `/repos/${repo}/actions/workflows/${WORKFLOW_FILES[buildVariant]}/dispatches`,
      "-f",
      `ref=${ref}`,
      "-f",
      `inputs[ninja_workers]=${maxJobs}`,
      "-F",
      `inputs[use_cache]=${useCache}`,
      "-F",
      `inputs[publish_release]=${publishRelease}`,
      "-F",
      `inputs[ck_disable_bwd]=${ckDisableBwd}`,
      "-f",
      `inputs[retry_count]=${String(nextRetryCount)}`,
    ],
    {
      env: process.env,
      stdio: "inherit",
      shell: false,
      encoding: "utf8",
    },
  );

  if (result.error) {
    console.warn(`gh api dispatch spawn failed: ${result.error.message}`);
    return false;
  }
  return result.status === 0;
}

export async function runWatchdogRetry(options: {
  buildVariant: keyof typeof WORKFLOW_FILES;
  abortMetaDir?: string;
  cacheMetaDir?: string;
  allOptDims?: string[];
}): Promise<void> {
  if (options.buildVariant === "parallel") {
    if (options.abortMetaDir === undefined) {
      throw new Error("parallel watchdog retry requires --abort-meta-dir");
    }
    if (options.cacheMetaDir === undefined || options.allOptDims === undefined) {
      throw new Error(
        "parallel watchdog retry requires --cache-meta-dir and --all-opt-dims",
      );
    }

    const evaluation = evaluateParallelWatchdogRetry({
      allOptDims: options.allOptDims,
      cacheMetaDir: options.cacheMetaDir,
      abortMetaDir: options.abortMetaDir,
    });
    if (!evaluation.eligible) {
      throw new Error(
        `Compile failed without eligible watchdog retry: ${evaluation.reason}`,
      );
    }

    console.log(
      `Watchdog abort metadata: ${evaluation.entries.map((entry) => entry.opt_dim).join(", ")}`,
    );
  } else {
    if (process.env.ABORT_TRIGGERED !== "true" || process.env.COMPILE_COMPLETE === "true") {
      throw new Error(
        "Watchdog retry for serial requires ABORT_TRIGGERED=true and COMPILE_COMPLETE!=true",
      );
    }
    if (process.env.ABORT_FORCE_KILLED === "true") {
      throw new Error(
        "Watchdog force-killed (ABORT_FORCE_KILLED=true); cannot retry",
      );
    }
  }

  if (requireGithubActionsEnv("USE_CACHE") !== "true") {
    throw new Error("Watchdog abort with use_cache=false; cannot retry");
  }

  const retryCountRaw = requireGithubActionsEnv("RETRY_COUNT");
  const retryCount = Number.parseInt(retryCountRaw, 10);
  if (!Number.isFinite(retryCount)) {
    throw new Error(`Invalid RETRY_COUNT: ${retryCountRaw}`);
  }
  if (retryCount >= MAX_RETRY_COUNT) {
    throw new Error(
      `Watchdog abort: retry_count=${retryCount} >= ${MAX_RETRY_COUNT}; giving up`,
    );
  }

  const nextRetry = retryCount + 1;
  let dispatched = false;

  for (let attempt = 1; attempt <= DISPATCH_ATTEMPTS; attempt++) {
    if (dispatchRetryWorkflow(options.buildVariant, nextRetry)) {
      dispatched = true;
      console.log(
        `Triggered ${options.buildVariant} retry run with retry_count=${nextRetry} (dispatch attempt ${attempt}/${DISPATCH_ATTEMPTS})`,
      );
      break;
    }
    console.warn(
      `gh api dispatch attempt ${attempt}/${DISPATCH_ATTEMPTS} failed`,
    );
    if (attempt < DISPATCH_ATTEMPTS) {
      await sleep(DISPATCH_RETRY_INTERVAL_MS);
    }
  }

  if (!dispatched) {
    throw new Error(
      `Failed to dispatch retry workflow after ${DISPATCH_ATTEMPTS} attempts`,
    );
  }

  console.log(
    `Waiting ${CANCEL_WAIT_MS / 1000}s for retry run to cancel this run via concurrency...`,
  );
  await sleep(CANCEL_WAIT_MS);
  throw new Error(
    `Retry run did not cancel this run within ${CANCEL_WAIT_MS / 1000}s`,
  );
}
