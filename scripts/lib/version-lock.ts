...
import { z } from "zod";
import { buildPipToolchainCacheKey, PIP_TOOLCHAIN_CACHE_FAMILY } from "./pip-cache-key.js";
...
type VersionLockVars = {
  ...
  ROCM_INDEX: string;
  ROCM_VERSION: string;
  PIP_TOOLCHAIN_CACHE_FAMILY: string;
...
function expectedWheelPattern(
  ...
  return `flash_attn-*+${filenameLocal}-${tag}-${tag}-win_amd64.whl`;
}

/** Ninja cache lockHash8：仅 toolchain + flash_attention + compile（不含 wheel/release）。 */
function ninjaCacheLockPayload(
  lock: Pick<
    z.infer<typeof versionLockSchema>,
    "toolchain" | "flash_attention" | "compile"
  >,
): string {
  return JSON.stringify({
    toolchain: lock.toolchain,
    flash_attention: lock.flash_attention,
...
export function readVersionLock(workspaceRoot: string): VersionLockVars {
  ...
  console.log(
    `VERSION.lock: python=${vars.PYTHON_VERSION} pytorch=${vars.PYTORCH_VERSION} GPU_ARCHS=${vars.GPU_ARCHS} ck_opt_dim=${vars.CK_OPT_DIM}`,
  );
